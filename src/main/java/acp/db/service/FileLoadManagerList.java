...
//    vField="to_number(extract(mssf_statistic,'statistic/records/all/text()').getstringval())";
  ...
  public List<FileLoadClass> queryPartOpen(int startPos, int cntRows) {
    openCursor(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
    cacheObj = fetchPart(startPos,cntRows);
    return cacheObj;
  }  

  public List<FileLoadClass> queryPartMove(int startPos, int cntRows) {
    cacheObj = fetchPart(startPos,cntRows);
    return cacheObj;
  }  
