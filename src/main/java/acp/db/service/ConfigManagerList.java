...
public class ConfigManagerList extends ManagerList {
  ...
  public List<ConfigClass> queryPartOpen(int startPos, int cntRows) {
    openCursor(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
    cacheObj = fetchPart(startPos,cntRows);
    return cacheObj;
  }  

  public List<ConfigClass> queryPartMove(int startPos, int cntRows) {
    cacheObj = fetchPart(startPos,cntRows);
    return cacheObj;
  }  
  ...
  private ConfigClass getObject(ResultSet rs) throws SQLException {
    ...
    int rsSourceId = rs.getInt("msso_msss_id");
