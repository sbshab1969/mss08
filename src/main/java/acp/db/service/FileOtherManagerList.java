...
public class FileOtherManagerList extends ManagerList {
  ...
  public List<FileOtherClass> queryPartOpen(int startPos, int cntRows) {
    openCursor(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
    cacheObj = fetchPart(startPos,cntRows);
    return cacheObj;
  }  

  public List<FileOtherClass> queryPartMove(int startPos, int cntRows) {
    cacheObj = fetchPart(startPos,cntRows);
    return cacheObj;
  }  
